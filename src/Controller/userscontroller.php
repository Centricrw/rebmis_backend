...
class UsersController
{
    ...
    {
        $input = (array) json_decode(file_get_contents('php://input'), true);
        if (!UserValidation::ValidateUpdatedUser($input)) {
            return Errors::unprocessableEntityResponse();
        }
        $data = new \stdClass();
        ...
        try {
            ...
            $this->usersModel->updateUser($input, $user_id, $decoded_data->data->id);

            $response['status_code_header'] = 'HTTP/1.1 200 OK';
            $response['body'] = json_encode(["message" => "User profile information updated successfully!"]);
            return $response;
        } catch (\Throwable $e) {
            return Errors::notAuthorized();
        }
    ...
    /**
     * Function to suspend user by id
     * @param {String} $user_id
     * @return {Json} $result
     */
    ...
    function activateUser($user_id)
    {
        ...
        try {
            ...
            if (sizeof($user) == 0) {
                return Errors::notFoundError("Action failed, This user is not suspended or disableled!");
            }
